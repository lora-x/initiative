...
def main():
    ...
    for submodel in dialogRPT_models:
        wandb.init(project=submodel, entity="lora")
        model_card = dialogRPT_models[submodel]
        ...
        training_args = TrainingArguments(submodel + "output", num_train_epochs=args.num_epoch, evaluation_strategy="epoch", learning_rate = args.learning_rate, report_to="wandb")
        ...
        train_all_predictions.append(train_logits)
        ...
        dev_all_predictions.append(dev_logits)
        # dev_pred = torch.clamp(torch.round(dev_logits), min = 0, max = 4)

    X_train = torch.stack((train_all_predictions[0], train_all_predictions[1], train_all_predictions[2]))
    X_train = torch.squeeze(X_train)
    X_train = X_train.tolist()
    X_dev = torch.stack((dev_all_predictions[0], dev_all_predictions[1], dev_all_predictions[2]))
    X_dev = torch.squeeze(X_dev)
    X_dev = X_dev.tolist()
    print (X_train)

    clf = MLPClassifier(alpha=1e-5, hidden_layer_sizes=(5), random_state=1)
    clf.fit(X_train, train_labels)
    Y_pred = clf.predict(X_dev)
    Y_pred = np.array(Y_pred)
    Y_pred = np.clip(Y_pred.round(), min = 0, max = 4)
    dev_accuracy = accuracy_score(dev_labels, Y_pred)
    dev_MSE = mean_squared_error(dev_labels, Y_pred)
    print ("dev acc: ", dev_accuracy, "  dev MSE: ", dev_MSE)
